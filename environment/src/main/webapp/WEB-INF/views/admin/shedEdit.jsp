  ...
  <body>
    ...
    <form class="form-horizontal" role="form" style="width: 1000px">
	  <div class="form-group">
	    ...
	    <div class="col-sm-4">
	      <input type="text" value="<%=shed.getCode() %>" class="form-control" id="code" placeholder="大棚编码">
	    </div>
	  </div>
	  ...
	  <div class="form-group">
	    ...
	    <div class="col-sm-4">
	      ...
	      <% for ( int i = 0 ; i < bases.size() ; i ++ ) { %>
	      	<option value="<%=bases.get(i).getId() %>" <%if(shed.getBase().getId() == bases.get(i).getId() ) { %>selected="selected"<%} %>><%=bases.get(i).getName() %></option>
	      <% } %>
	      </select>
	    </div>
	  </div>
	...
	<script type="text/javascript">
		$("#update").click(function () {
			...
			$.ajax({
				url: '<%=path%>/admin/shedEdit.do' ,
				method: 'POST' ,
				async: false ,
				data: {id:<%=shed.getId()%>, baseId:base, code:code} ,
				dataType: 'JSON' ,
