  ...
  <body>
    ...
    <form class="form-horizontal" role="form" style="width: 1000px">
	  ...
	  <div class="form-group">
	    ...
	    <div class="col-sm-4">
	      <select id="corp" style="height:34px;width:100%; text-align:center">
	      	<option value="0">--请选择作物--</option>
	      	<% for ( int i = 0 ; i < corps.size(); i ++ ) { %>
	      		<option value="<%=corps.get(i).getId()%>"><%=corps.get(i).getName() %></option>
	      	<% } %>
	      </select>
	    </div>
	  </div>
...
<!--     </div> container -->
	...
	<script type="text/javascript">
		$("#update").click(function () {
			...
			$.ajax({
				url: '<%=path%>/admin/shed/addCorp.do' ,
				method: 'POST' ,
				async: false ,
				data: {id:id,corpId:corpId} ,
				dataType: 'JSON' ,
